...
socket.on('join_room_response', (payload) =>{
	...
	/* If we are being noified of ourselves, then ignore the message and return */
	if (payload.socket_id === socket.id){
		return;
	}

	let domElements = $('.socket_'+payload.socket_id);
	/* If we are repeat notified, then return */
	if (domElements.length !== 0 ){
		return;
	}
	/*
		<div class="row align-items-center"> nodeA
	...
	*/
	...
	nodeB.append('<h4>'+payload.username+'</h>');
